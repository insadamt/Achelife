...
Income and Expense composition uses a searchable visual Category picker. Category icons are derived from existing preset keys and names, so custom, renamed, archived, imported, and older Categories remain compatible without a new persisted format. Choosing a parent with children opens its Subcategory view, including an explicit no-Subcategory option. Phase 19 adds one optional reusable Merchant and up to ten reusable colored Tags to Income and Expense transactions; Transfers keep Account endpoints as their only cross-account identity.
...
`/money/history` supports pagination, date-grouped results, visible applied-filter chips, and filters for type, Account, parent Category, parent-scoped Subcategory, Merchant, Tag, and date range. Escaped text search covers notes, Category/Subcategory names, Merchant names, and Tag names. Positive Transfer fees are projected under `Financial → Bank Fees` for search and categorization filters without setting transaction Category foreign keys, creating another transaction, or applying another balance effect. This projection is the reporting contract for later Statistics work.
