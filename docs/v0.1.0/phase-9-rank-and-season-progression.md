...
Phase 14 presents the immutable final Rank snapshot beside signed final Season SP and authoritative module contributions. Negative SP remains Unranked, zero/no activity remains Bronze I, and Legend has no artificial ceiling. Previous-Season comparison never recalculates or overwrites either Rank snapshot.

## v1 portability extension

Phase 15 records latest Rank and signed SP in the manifest, exports every Season snapshot, and validates stored SP against Task, Habit, Diary, active Objective, and Constitution contributions. Catch-up applies exact deltas only through original Day 30; Money and Subscription restoration remains excluded. Ended imports finalize Rank before Welcome Back links to closeout.
...
Rank tests cover threshold boundaries, division order, normal progress, negative SP, Legend behavior, live promotion and demotion through existing module actions, reversal demotion, completed snapshots, missing-snapshot backfill, new-Season reset, Money isolation, and internal ownership scoping.
