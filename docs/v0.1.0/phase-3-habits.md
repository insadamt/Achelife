...
## Habit card and statistics UI refinement

Habit cards keep their collapsed summary focused on identity, current streak, and a labeled Statistics action. Difficulty, base reward, schedule, and Numeric target remain available through editing instead of appearing on the card. The collapsed strip shows today and the preceding six local calendar days, oldest first, with Today always at the right edge. Weekday labels follow those dates. A visible Show season / Last 7 days control switches views. Recent outcomes from previous seasons remain visible and read-only; dates before creation and during intermission are inactive. The expanded season grid retains its Monday–Sunday layout. Action menus support outside-click dismissal and Escape with focus restoration. Cards stack on mobile and retain the two-column calendar layout on desktop. Per-habit statistics and their interaction rules are documented in [Habit statistics](../habit-statistics.md).
