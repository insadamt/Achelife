...
- The SP trajectory attributes completed Tasks and Objectives in the user's saved timezone. Habit, Diary, and Constitution activity uses its stored local calendar date. Its **Cumulative** view shows the running Season balance, while **Daily SP** shows the net SP attributed to each individual day. Both views preserve negative SP.
- The trajectory follows the Task statistics chart pattern: every displayed day is keyboard-focusable and reveals its daily SP change and cumulative total on hover or focus.
