...
Focus by Project appears in its own full-width section as an interactive donut chart and a color-matched breakdown showing each Project's completed Focus Time and share of the selected period. Hovering a segment enlarges it and reveals its Project details. Every Project has a unique saved color; choosing a color already used by another Project assigns a distinct color instead. Inbox uses the Tasks accent color. Focus by Project and Most-focused Tasks use each Task's current organization when the page is requested. Historical Focus therefore follows a Task when it moves, uses the Project's current name and color after an edit, and moves to Inbox when its Project is deleted. Inbox combines all unassigned Tasks. Deleting a Task cascades to its Focus Sessions, removing it from every Focus total and ranking. The page does not store Project or Task-name snapshots on Focus Sessions.
