...
export function MoneyQuickActions({ disabled = false, onSelect, stacked = false }: { disabled?: boolean; onSelect: (type: MoneyTransactionType) => void; stacked?: boolean }) {
    return (
        <div className={stacked ? 'grid grid-cols-3 gap-2 xl:grid-cols-1' : 'grid grid-cols-3 gap-2 sm:gap-3'}>
            {actions.map((action) => {
                ...
                return (
                    <button
                        className={classNames(
                            'focus-ring group flex min-h-16 items-center gap-3 rounded-2xl border border-border-strong bg-elevated px-3 py-3 transition-[transform,background-color,border-color] hover:-translate-y-0.5 hover:border-[color-mix(in_srgb,var(--money-accent)_45%,var(--border-strong))] hover:bg-surface-hover disabled:pointer-events-none disabled:opacity-45 sm:min-h-18 sm:px-4',
                            stacked ? 'justify-center text-center xl:justify-start xl:text-left' : 'justify-center text-center',
                        ...
                        type="button"
                    >
                        <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-app/70">
                            <ActionIcon
                                aria-hidden="true"
                                className={classNames(
                                    'text-accent-ink',
