...
export function AccountFormDrawer({ account = null, onClose }: { account?: MoneyAccountData | null; onClose: () => void }) {
    ...
    return (
        <Drawer description={account ? 'The name remains editable. Financial foundations lock after first use.' : 'Add one real-world place where you keep money.'} onClose={onClose} open title={account ? 'Edit Account' : 'New Account'}>
            <form className="space-y-6" onSubmit={submit}>
                ...
                <Field disabled={locked} error={form.errors.initial_balance} inputMode="decimal" label="Initial balance" onChange={(event) => form.setData('initial_balance', event.target.value)} required value={form.data.initial_balance} />
                {locked && <p className="rounded-2xl border border-warning/30 bg-warning/8 px-4 py-3 text-sm text-warning">Currency and initial balance are locked because this Account has financial history.</p>}
                <Button disabled={form.processing} fullWidth type="submit">{account ? 'Save Account' : 'Create Account'}</Button>
            </form>
        </Drawer>
    );
}
